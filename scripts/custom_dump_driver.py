...
class CustomDump(object):
    ...
    def getOutfiles(cls, logfile):
        ...
        :rtype: dict
        """
        task = sh.grep(f'{FlAG_TASK[1:]}:', logfile)
        tasks = task.split()[-1].strip("'][").split(',')
        names = {x: cls.NAME[x] for x in tasks}
        outfiles = {x: sh.grep('-i', y, logfile) for x, y in names.items()}
        outfiles = {
            x: re.compile(re.escape(names[x]), re.IGNORECASE).sub('', y)
            for x, y in outfiles.items()
        }
        outfiles = {x: z for x, y in outfiles.items() for z in y.split() \
                    if z.endswith(cls.DATA_EXT % x)}
        return outfiles

    @classmethod
    def combine(cls, files, log, name):
        """
