...
class Nemd(object):
    ...
    def loadLammpsIn(self):
        ...
        self.lammps_units = self.lammps_in.cmd_items[
            fileutils.LammpsInput.UNITS]
        log(f"{self.lammps_units} is the units.")
        self.timestep = self.lammps_in.cmd_items[
            fileutils.LammpsInput.TIMESTEP]
        log(f"{self.timestep} is the timestep.")

    def loadTemp(self):
        block_num = 5
        self.temp_data, temp_dat_num = fileutils.load_temp(
            self.options.temp_file, block_num=block_num)
    ...
    def plot(self):
        temp_ene_plotter = plotutils.TempEnePlotter(self.temp_data,
                                                    self.lammps_energy.data,
                                                    self.jobname)
